...
fn test_adding_rooms_1() {
    ...
    dungeon.set_link("Hallway", Direction::South, "Laboratory").unwrap();
    ...
    assert_eq!(dungeon.get_next_room("Hallway", Direction::South).unwrap().unwrap().name, "Laboratory");
    assert_eq!(dungeon.get_next_room("Laboratory", Direction::North).unwrap().unwrap().name, "Hallway");
}
